...
(function () {
  ...
  var DRAW = [
    ...
    '  emi=vec3(1.,.32,.04)*heat*heat*1.6+vec3(1.,.75,.35)*pow(heat,6.)*1.2;',
    '  emi+=vec3(1.,.45,.1)*smoothstep(band*1.8,band,dist)*step(along,F)*exp(-age*18.)*.35;',
    ' }',
    ' if(M>.5&&M<1.5){',
    ...
    ' if(M<.5){',
    '  float sl=fract(T*.07)*1.5-.25;',
    '  float g=exp(-abs(v.y-sl)*420.);',
    '  c+=vec3(.01,.99,.12)*(g*.55+exp(-abs(v.y-sl)*26.)*smoothstep(.2,0.,cr.r)*step(F,along)*.9);',
    ' }',
